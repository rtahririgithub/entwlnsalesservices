...
public class FIFAGetAvailableProductItemDelegate {

	private static Logger logger = Logger.getLogger(FIFAGetAvailableProductItemDelegate.class);

	ProductItemDelegateV2 productItemDelegate = new ProductItemDelegateV2();
	ProductGroupCategorizationDelegateV2 groupDelegate = new ProductGroupCategorizationDelegateV2();
	...
	private class ProductLevelObjects {
		...
		//FIFA-SHS product level characteristic
	...
	public GetAvailableProductItemDelegateResponse execute(String transactionId, ShoppingCartContextVO shoppingCartContextVO ){
		...
		OperationHeaderVO operationHeader = new OperationHeaderVO();
		operationHeader.setSalesTransactionId(transactionId);
		shoppingCartContextVO.getShoppingCartVO().setOperationHeader(operationHeader);
...
//		}
		...
		for(ProductLevelObjects productObjects: productLevelObjectsMap) {
			...
			//FIFA-SHS
			if(EnterpriseWLNSalesServicesConstants.HOME_SECURITY.equalsIgnoreCase(productObjects.getProductTypeCode())) {
				
				String productType = productObjects.getProductTypeCode();
				isLiketoLikeRecontract = WirelineProductHelper.isLikeToLikeReContracting(productType, shoppingCartContextVO, productCataloguesFromGrid);
				GetAvailableProductItemDelegateResponse availablePorductItemDelegateResponse = this.createResponse(productType, offer, sweeteners, isLiketoLikeRecontract, shoppingCartContextVO);
				if (availablePorductItemDelegateResponse.getErrorResponse() != null)
					return availablePorductItemDelegateResponse;
				else {
					List<AvailableHomeSecurityProductItemVO> availProdItems = availablePorductItemDelegateResponse.getResponse().getHomeSecurityProductItems();
					AvailableHomeSecurityProductItemVO availableProductItem = availProdItems.get(0);
					List<AvailableFFHEquipmentTypeVO> equipments = availableProductItem.getEquipments();
					if (equipments == null) equipments = new ArrayList<AvailableFFHEquipmentTypeVO>();
					List<FFHEquipmentItemVO> equipmentItems = productItemDelegate.extractEquipmentItems(equipments);
					Product assignedInternetProduct = null;
					if (assignedProducts != null) {
						assignedInternetProduct = WirelineProductHelper.getAssignedProducts(assignedProducts, productType);
						if (assignedInternetProduct != null) 
						   {
							addExistingEquipment(equipments, assignedInternetProduct, offer.getOfferId(), isLiketoLikeRecontract, productCataloguesFromGrid, productType);
						    if (!equipments.isEmpty()) {
							 availableProductItem.setEquipments(equipments);
						    } 
					    }    
					...
					filterAvailableEquipmentDeliveryMethod(equipments, isInstallerRequired);
					// Don't pass the salesOfferDetailVO and sweetenerVO as the ProductItems and FFHEquipmentItems already have all the info
					ProductItemDelegateResponse productItemResponse = productItemDelegate.populateProductItems(productType, availablePorductItemDelegateResponse.getProductItems(), equipmentItems, null, null);				
					List<ProductItemVO> productItems = productItemResponse.getProductItems();
					filterDiscountProductItems( productItems, shoppingCartContextVO, productType );
					List<ProductItemConstraintGroupVO> productItemConstraintGroups = constraintDelegate.groupProductItemConstraints(productType, productItems, offer);

					productItemDelegate.infuseEquipmentItems(productItemResponse.getEquipmentItems(), equipments);
					
					List<ProductItemGroupCategoryVO> productItemGroupCategories = groupDelegate.groupProductItems(productItems);
					
					availableProductItem.setProductItemGroupCategories(productItemGroupCategories);
					availableProductItem.setProductItemConstraintGroupList(productItemConstraintGroups);
					apiResp.getHomeSecurityProductItems().addAll(availProdItems);
					delegResponse.getProductItems().addAll(productItems);
				}
			}
			...
			if(EnterpriseWLNSalesServicesConstants.TTV.equalsIgnoreCase(productObjects.getProductTypeCode())) {
				...
				if (ttvResponse.getErrorResponse() != null)
					...
					List<ProductItemVO> productItems = productItemResponse.getProductItems();
					ProductItemDelegateResponse productItemResponseForTTV = productItemDelegate.populateProductItems(EnterpriseWLNSalesServicesConstants.TTV, productItemResponse.getProductItems(), equipmentItems, null, null);				
...
//		}
	...
	private List<ProductLevelObjects> buildProductLevelObjects(ShoppingCartContextVO shoppingCartContextVO) {
		...
		if(!CollectionUtils.isEmpty(cart.getCartItemList())){
			for (CartItemVO cartItem : cart.getCartItemList()) {
				...
				Offer offer = null;
				FIFAProductOfferingsVO getSalesOfferDetailResponseVO =  new FIFAProductOfferingQualificationDelegate().getProductOfferingQualification( cart );
				...
				if (cartItemProducts.getInternetProduct() != null) {
					...
					copyCartSweentersToContext(shoppingCartContextVO, cartItemProducts.getInternetProduct(), productType);

					populateMap(shoppingCartContextVO, result, cartItemRelationId, offer, productType);
					
				}
				if (cartItemProducts.getTelevisionProduct() != null) {
					String productType = EnterpriseWLNSalesServicesConstants.TTV;

					copyCartSweentersToContext(shoppingCartContextVO, cartItemProducts.getTelevisionProduct(), productType);
				...
				if (cartItemProducts.getHomePhoneProduct() != null) {
					...
					copyCartSweentersToContext(shoppingCartContextVO, cartItemProducts.getHomePhoneProduct(), productType);
				...
				//FIFA-SHS
				if (cartItemProducts.getHomeSecurityProduct() != null) {
					String productType = EnterpriseWLNSalesServicesConstants.HOME_SECURITY;
					copyCartSweentersToContext(shoppingCartContextVO, cartItemProducts.getHomeSecurityProduct(), productType);
	...
	/** 
	 * In Atlas phase I, we do not call Promotion qualification to retrieve promotions. 
	 * This method just copy the sweeteners from shoppingCart to shoppoingCartContextVO.availableSweetenerOfferListResponseVO,
	 * to make them look like being retrieved from downstream, so that they will be appear in the final response.
	 * 
	 * @param scCtxVO
	 * @param productBaseVO
	 * @param productType
	 */
	private void copyCartSweentersToContext ( ShoppingCartContextVO scCtxVO, ProductTypeBaseVO productBaseVO , String productType  ) {
		
		for (FFHSweetenerTypeVO sweetener : productBaseVO.getSweeteners()) {
			
			if (sweetener.getPrice() != null) {
			
				checkForNullorEmptySweetenerLists(scCtxVO);
				
				List<Description> discountMarketingNames = new ArrayList<Description>();
				if (CollectionUtils.isNotEmpty(sweetener.getRelatedPromotionList())) {
					for (RelatedImmediatePromotionVO relatedImmediatePromotion : sweetener.getRelatedPromotionList()) {
						Description description = new Description();
						description.setLocale("EN");
						description.setDescriptionText(relatedImmediatePromotion.getName());
						discountMarketingNames.add(description);
					}
				}
				
				DiscountProductCatalogueItem discountProductCatalogueItem = new DiscountProductCatalogueItem();
				discountProductCatalogueItem.setDiscountPriceAmt(	(double) sweetener.getPrice().getBasePriceAmount().getValueAmt());
				
				ProductCatalogueIdentifier discountCatalogueIdentifier = new ProductCatalogueIdentifier();
				
				ProductComponentVO  sweenterDiscountIdentifier = sweetener.getDiscounts().get(0).getProductCatalogueIdentifiers().get(0);
				discountCatalogueIdentifier.setProductCatalogueId( sweenterDiscountIdentifier.getProductCatalogueId() ); //fake Id
				discountProductCatalogueItem.setProductCatalogueIdentifier( discountCatalogueIdentifier);

				//QC84026, preserve discount's parent catalogueId, which is used to indentify which productOffering it associated to   
				ProductCatalogueIdentifier discountParenCatalogueIdentifier = new ProductCatalogueIdentifier();
				discountParenCatalogueIdentifier.setProductCatalogueId( sweenterDiscountIdentifier.getParentProductCatalogueId() ); 
				discountProductCatalogueItem.setParentProductCatalogueIdentifier(discountParenCatalogueIdentifier);
				
				if (sweetener.getPrice().getRecurrenceCount()!=null ) {
					discountProductCatalogueItem.setRecurringCount(	BigInteger.valueOf(sweetener.getPrice().getRecurrenceCount()));
				}
				...
				discountProductCatalogueItem.setProductCatalogueNameList(discountMarketingNames);


				Discount discount = new Discount();
				discount.setIncludedInd(true);
				discount.setPreSelectedDiscountInd(true);
				discount.setMarketingNameList(discountMarketingNames);
				discount.setDiscountProductCatalogueItemList(Arrays.asList(discountProductCatalogueItem));

				if (  sweetener.getDiscounts()!=null ) {
					...
					for (FFHDiscountTypeVO fhhDiscountType : sweetener.getDiscounts()) {
						if(	CollectionUtils.isNotEmpty(fhhDiscountType.getProductCatalogueIdentifiers())) {
				...
				wirelineOfferProductSummary.setProductTypeCode(productType);
				wirelineOfferProductSummary.setDiscountList( Arrays.asList(  discount) );
				...
				sweetenerOfferSummary.setPromotionId( sweetener.findPromotionId() ); //fake Id
				sweetenerOfferSummary.setOfferProductSummary(offerProductSummary);

				// append to AvailableSweetenerOfferList
				scCtxVO.getAvailableSweetenerOfferListResponseVO().getSweetenerOfferSummaryList().add(sweetenerOfferSummary);
			}
		}
	}
	
	private void checkForNullorEmptySweetenerLists( ShoppingCartContextVO scCtxVO ) {
